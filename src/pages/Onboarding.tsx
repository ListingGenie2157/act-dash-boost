...
export default function Onboarding() {
  ...
  // Fix: control the visible calendar month + allow nav across months/years
  ...
  const handleSetBaseline = async () => {
    ...
    try {
      const { error } = await supabase.functions.invoke('set-baseline', {
        body: { scores: numericScores, notes: notes.trim() || undefined }
      });
      if (error) throw error;
  ...
  const renderStep1 = () => (
    <Card>
      ...
      <CardContent className="space-y-4">
            ...
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar
                ...
                onSelect={(d) => {
                  if (!d) return;
                  setTestDate(d);
                  // keep the month in sync with selected date for a better UX
                  setCalendarMonth(d);
                }}
                // Fixes: enable visible month navigation and month/year dropdowns
                month={calendarMonth}
        ...
        <Button onClick={handleSetTestDate} disabled={!testDate || loading} className="w-full">
  ...
  const renderStep2 = () => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Enter any previous ACT scores. Skip sections you haven't taken.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
