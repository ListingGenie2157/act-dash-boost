...
export default function ImportE1C() {
...
<p>10. rebuilt. Completed past action last summer.</p>`,
        ...
        setStatus('success');
        toast.success('Lesson E1.C imported successfully!');
        
        // Wait a moment then redirect
        setTimeout(() => {
          navigate('/lesson/E1.C');
        }, 1500);
      } catch (error) {
        console.error('Import failed:', error);
        setStatus('error');
        toast.error('Failed to import lesson');
      }
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-background">
      <div className="text-center space-y-4">
        {status === 'importing' && (
          <>
            <LoadingSpinner />
            <p className="text-muted-foreground">Importing lesson E1.C...</p>
          </>
        )}
        {status === 'success' && (
          <>
            <div className="text-6xl">✅</div>
            <p className="text-foreground font-medium">Import successful!</p>
            <p className="text-muted-foreground text-sm">Redirecting to lesson viewer...</p>
          </>
        )}
        {status === 'error' && (
          <>
