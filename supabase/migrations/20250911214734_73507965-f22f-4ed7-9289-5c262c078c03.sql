...
CREATE POLICY IF NOT EXISTS "Skills are viewable by authenticated users" 
...
CREATE POLICY IF NOT EXISTS "Questions are viewable by authenticated users" 
...
CREATE POLICY IF NOT EXISTS "Users can view their own review queue" 
ON public.review_queue FOR SELECT 
TO authenticated 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own review items" 
ON public.review_queue FOR INSERT 
TO authenticated 
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own review items" 
ON public.review_queue FOR UPDATE 
TO authenticated 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete their own review items" 
ON public.review_queue FOR DELETE 
TO authenticated 
USING (auth.uid() = user_id);

-- Error bank policies
CREATE POLICY IF NOT EXISTS "Users can view their own error bank" 
ON public.error_bank FOR SELECT 
TO authenticated 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own errors" 
ON public.error_bank FOR INSERT 
TO authenticated 
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own errors" 
ON public.error_bank FOR UPDATE 
TO authenticated 
USING (auth.uid() = user_id);

-- Study plan days policies
CREATE POLICY IF NOT EXISTS "Users can view their own study plans" 
ON public.study_plan_days FOR SELECT 
TO authenticated 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own study plans" 
ON public.study_plan_days FOR INSERT 
TO authenticated 
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own study plans" 
ON public.study_plan_days FOR UPDATE 
TO authenticated 
USING (auth.uid() = user_id);

-- Study tasks policies
CREATE POLICY IF NOT EXISTS "Users can view their own study tasks" 
ON public.study_tasks FOR SELECT 
TO authenticated 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own study tasks" 
ON public.study_tasks FOR INSERT 
TO authenticated 
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own study tasks" 
