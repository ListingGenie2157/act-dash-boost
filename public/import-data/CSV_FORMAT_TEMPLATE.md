# ACT Question Import CSV Format Template
...
1. **CSV Format**: Use COMMA characters to separate columns
2. **Quoted Fields**: Wrap fields containing commas, quotes, or newlines in double quotes
3. **Escaping Quotes**: Use double quotes ("") to escape quotes within quoted fields
4. **Boolean Values**: Use TRUE/FALSE (case-insensitive)
5. **JSON Fields**: For `chart_images` and `marked_text`, use valid JSON syntax wrapped in quotes
6. **Passage Data**: Only include passage fields (passage_title, passage_text, etc.) on the FIRST question of that passage
7. **Empty Fields**: Leave empty for optional fields (just use consecutive commas)
8. **Line Breaks**: For multi-line text within fields, wrap the entire field in quotes
...
4. Click "Import" to load into staging_items table
